...
public class PrisonPoliceDatabase {
	...
	public PrisonPoliceDatabase(PrisonPolice plugin, Logger logger,
			String pluginName, String path, String dbName, String extension) {
	...
	public void checkBlock(Block block, Player player) {
		...
		try {
			ResultSet rs = _sqLite
					.query("SELECT * FROM block WHERE uuid = (SELECT uuid FROM block "
							+ "WHERE world = '"
							+ block.getWorld().getName()
							+ "' "
							+ "AND x = '"
			...
			while (rs.next()) {
				try {
					if (first) {
						...
						addEvidenceList(pID);
						player.sendMessage(plugin.processColors("&c"
								+ plugin.getConfig().getString("blocksname")
								+ " owned by: "
								+ plugin.getServer().getOfflinePlayer(pID)
										.getName()
								+ ". Added to evidence list..."));
						Player p = plugin.getServer().getPlayer(pID);
						if (p != null)
							p.sendMessage(plugin
									.processColors("&cYou are now wanted by the police. Tread lightly."));
					}
					first = false;
	...
	public void addEvidenceList(UUID id) {
		try {
			_sqLite.query("INSERT INTO evidencelist(uuid,username) SELECT '"
					...
					+ "' WHERE NOT EXISTS (SELECT * FROM evidencelist WHERE uuid = '"
					+ id.toString() + "') ;");
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public void removeEvidenceList(UUID id) {
		try {
			_sqLite.query("DELETE FROM evidencelist WHERE uuid = '"
					+ id.toString() + "';");
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public boolean isOnList(UUID id) {
		try {
			ResultSet checklist = _sqLite
					.query("SELECT * FROM evidencelist WHERE uuid = '"
							+ id.toString() + "';");
			if (checklist.next()) {
				return true;
			}
	...
	public String evidenceList() {
		...
		try {
			...
			while (wanted.next()) {
				...
				listString += plugin.getServer().getOfflinePlayer(uuid)
						.getName()
						+ ", ";
			}
			if (listString != "") {
